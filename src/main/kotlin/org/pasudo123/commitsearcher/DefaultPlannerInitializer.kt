...
class DefaultPlannerInitializer(
    ...
    fun process() {
        // # 1
        val firstPlanForPlannerDomain = Planner(title = "플래너 레파지토리 구현하기", description = "플래너 도메인 모듈 설정 및 부가적인 것 설정")
        plannerRepository.save(firstPlanForPlannerDomain)

        val issueTickets: List<IssueTicket> = listOf(
            IssueTicket(title = "플래너 도메인 작성", description = "")
                .apply { this.setCurrentPlanner(firstPlanForPlannerDomain) },
        ...
        // # 2
        val secondPlanForPlannerDomain = Planner(title = "롯데마트 가기", description = "롯데마트를 가서 필요한 물품들을 구매한다.")
        plannerRepository.save(secondPlanForPlannerDomain)

        val issueTickets2: List<IssueTicket> = listOf(
            IssueTicket(title = "치실 구매하기", description = "")
                .apply { this.setCurrentPlanner(secondPlanForPlannerDomain) },
